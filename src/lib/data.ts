...
export async function getGoalTrees(): Promise<GoalTreeFile> {
  try {
    const raw = await readFile(filePath("goal-trees.json"), "utf-8");
    return JSON.parse(raw) as GoalTreeFile;
  } catch {
    return { trees: [] };
  }
}

export async function saveGoalTrees(data: GoalTreeFile): Promise<void> {
  await fileMutexes.goalTrees.runExclusive(async () => {
    await _writeJson("goal-trees.json", data);
  });
}

export async function mutateGoalTrees<T>(fn: (data: GoalTreeFile) => Promise<T>): Promise<T> {
  return fileMutexes.goalTrees.runExclusive(async () => {
    let data: GoalTreeFile;
    try {
      const raw = await readFile(filePath("goal-trees.json"), "utf-8");
      data = JSON.parse(raw) as GoalTreeFile;
    } catch {
      data = { trees: [] };
    }
    const result = await fn(data);
